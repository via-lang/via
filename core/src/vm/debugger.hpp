...
using CommandHandler =
    std::function<bool(const std::vector<CommandArgument>& args)>;
...
class CommandTable {
 public:
  CommandTable(Logger& logger) : m_logger(logger) {}

 public:
  void register_command(std::string name, std::string help,
                        std::vector<ArgumentType> args,
                        CommandHandler handler) {
...
class Debugger final {
 public:
  explicit Debugger(VirtualMachine& vm) : m_vm(vm), m_cmds(m_logger) {}
