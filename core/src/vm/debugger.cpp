...
static via::CommandArgument parse_argument(const std::string& tok) {
  if (tok == "true" || tok == "on") return true;
  if (tok == "false" || tok == "off") return false;
  ...
  if (has_digit && (has_dot || has_exp)) {
    ...
    if (end != tok.c_str() && *end == '\0') return val;
  }
...
static std::vector<via::CommandArgument> parse_arguments(
...
static bool validate_command(const via::Command& command,
                             const ActiveCommand& active) {
  // This should never happen in practice but we check for it anyways
  if (command.name != active.name) return false;
  if (command.args.size() != active.args.size()) {
    command.logger->error("missing arguments (expected {}, got {})",
                          command.args.size(), active.args.size());
    return false;
  }
...
void via::CommandTable::print_help() const {
  ...
  for (const auto& [_, cmd] : m_commands) {
    ...
    for (const auto& type : cmd.args) oss << " [" << to_string(type) << "]";
  ...
  for (const auto& [name, cmd] : m_commands) {
    ...
    std::cout << "  " << std::left << std::setw(static_cast<int>(max_name_len))
              << name << " " << std::left
              << std::setw(static_cast<int>(max_args_len)) << oss.str()
              << "     ->  " << cmd.help << "\n";
  }
...
void via::Debugger::register_default_commands() noexcept {
  ...
  m_cmds.register_command("help", "prints the help menu", {},
                          [&](const auto& args) {
                            m_cmds.print_help();
                            return true;
                          });
  ...
  m_cmds.register_command(
      "step", "steps the interpreter a given times", {ArgumentType::INTEGER},
      [&](const auto& args) {
  ...
  m_cmds.register_command(
      ...
      [&](const auto& args) {
        while (true) {
          ...
          std::println("0x{:0>4x}  {}", counter * 8,
                       m_vm.m_pc->to_string(true, counter));

          if (m_vm.m_pc->op == OpCode::HALT) break;
          m_vm.execute_once();
        }
        return true;
      });
  ...
  m_cmds.register_command(
      "pcat", "display program counter information at the given address",
      {ArgumentType::INTEGER}, [&](const auto& args) {
  ...
  m_cmds.register_command(
      "reg", "dumps the given register", {ArgumentType::INTEGER},
      [&](const auto& args) {
  ...
  m_cmds.register_command(
      "regs", "dumps all occupied registers", {}, [&](const auto& args) {
        bool debounce = true;
        size_t index = 0;
        Snapshot snapshot(&m_vm);
        ...
        for (const auto& reg : snapshot.registers) {
          if (reg != nullptr) {
            std::println(
                "R{} [{}]: {}", index,
                ansi::format(std::format("@0x{:0>16x}", (uintptr_t)reg),
                             ansi::Foreground::NONE, ansi::Background::NONE,
                             ansi::Style::FAINT),
                reg->to_string());
            debounce = true;
  ...
  m_cmds.register_command(
      "jump", "jumps to the given program counter", {ArgumentType::INTEGER},
      [&](const auto& args) {
        size_t pc = std::get<size_t(ArgumentType::INTEGER)>(args.at(0));
        if (pc % 8 == 0) {
          auto realpc = pc / 8;
  ...
  while (true) {
    ...
    if (auto command = m_cmds.find_command(active.name)) {
      if (validate_command(*command, active))
        if (!command->handler(active.args)) break;
      continue;
    }
