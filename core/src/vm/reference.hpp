...
class ValueRef final {
 ...
 public:
  ValueRef(VirtualMachine* vm) : m_ptr(nullptr) {}
  ValueRef(VirtualMachine* vm, Value* ptr) : m_ptr(ptr) {}

  template <typename... Args>
  explicit ValueRef(VirtualMachine* vm, Args&&... args)
      : m_ptr(Value::create(vm, std::forward<Args>(args)...)) {}

  ValueRef(const ValueRef& other) : m_ptr(other.m_ptr) {
    if (!other.is_null()) {
      other.m_ptr->m_rc++;
    }
  }

  ValueRef(ValueRef&& other) : m_ptr(other.m_ptr) { other.m_ptr = nullptr; }
