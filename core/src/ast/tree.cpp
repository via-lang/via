...
std::string via::ast::Path::to_string() const {
  return via::to_string(
      path, [](const auto& node) { return node->to_string(); }, "", "", "::");
}
...
std::string via::ast::ExprBinary::to_string(size_t depth) const {
  return std::format("({}{}{})", lhs->to_string(), op->to_string(),
                     rhs->to_string());
}
...
std::string via::ast::ExprCall::to_string(size_t depth) const {
  return std::format(
      "{}{}", callee->to_string(),
      via::to_string(
          args, [](const auto& arg) { return arg->to_string(); }, "(", ")"));
}
...
std::string via::ast::ExprLambda::to_string(size_t depth) const {
  return std::format(
      "fn {} -> {} {}",
      via::to_string(
          parms, [](const auto& arg) { return arg->to_string(); }, "(", ")"),
...
std::string via::ast::StatImport::to_string(size_t depth) const {
  return INDENT(depth) +
         std::format("import {}",
                     via::to_string(
                         path, [](const auto& arg) { return arg->to_string(); },
                         "", "", "::"));
}
...
std::string via::to_string(const via::SyntaxTree& tree) {
  std::ostringstream oss;
  oss << via::ansi::format("[disassembly of program AST]:\n",
                           ansi::Foreground::YELLOW, ansi::Background::NONE,
                           ansi::Style::UNDERLINE);

  for (const auto& node : tree) oss << node->to_string(1) << ";\n";
  return oss.str();
}
