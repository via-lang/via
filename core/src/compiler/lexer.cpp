...
static constexpr TokenReprPair KEYWORDS[] = {
    ...
    {"using", KW_USING},   {"none", KW_NONE},     {"bool", KW_BOOL},
    {"int", KW_INT},       {"float", KW_FLOAT},   {"string", KW_STRING},
};

static constexpr TokenReprPair OPERATORS[] = {
    {".", PERIOD},          {",", COMMA},        {";", SEMICOLON},
    {":", COLON},           {"::", COLON_COLON}, {"->", ARROW},
    {"?", QUESTION},        {"+", OP_PLUS},      {"-", OP_MINUS},
    {"*", OP_STAR},         {"/", OP_SLASH},     {"**", OP_STAR_STAR},
    {"%", OP_PERCENT},      {"&", OP_AMP},       {"~", OP_TILDE},
    {"^", OP_CARET},        {"|", OP_PIPE},      {"<<", OP_SHL},
    {">>", OP_SHR},         {"!", OP_BANG},      {"++", OP_PLUS_PLUS},
    {"--", OP_MINUS_MINUS}, {"&&", OP_AMP_AMP},  {"||", OP_PIPE_PIPE},
    {"<", OP_LT},           {">", OP_GT},        {"..", OP_DOT_DOT},
    {"(", PAREN_OPEN},      {")", PAREN_CLOSE},  {"[", BRACKET_OPEN},
    {"]", BRACKET_CLOSE},   {"{", BRACE_OPEN},   {"}", BRACE_CLOSE},
    {"=", OP_EQ},           {"==", OP_EQ_EQ},    {"+=", OP_PLUS_EQ},
    {"*=", OP_STAR_EQ},     {"/=", OP_SLASH_EQ}, {"**=", OP_STAR_STAR_EQ},
    {"%=", OP_PERCENT_EQ},  {"&=", OP_AMP_EQ},   {"^=", OP_CARET_EQ},
    {"|=", OP_PIPE_EQ},     {"<<=", OP_SHL_EQ},  {">>=", OP_SHR_EQ},
    {"!=", OP_BANG_EQ},     {"<=", OP_LT_EQ},    {">=", OP_GT_EQ},
    {"..=", OP_DOT_DOT_EQ},
};
...
via::Token* via::Lexer::read_identifier() {
  ...
  for (const auto& kw : KEYWORDS) {
    if (strlen(kw.str) != token->size) continue;
...
std::string via::to_string(const via::TokenTree& tt) {
  ...
  oss << via::ansi::format("[disassembly of tokens]:\n",
