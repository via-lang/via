...
std::string via::ir::TrContinue::to_string(const SymbolTable* sym_tab,
                                           size_t depth) const {
  return INDENT(depth) + "CONTINUE";
}

std::string via::ir::TrBreak::to_string(const SymbolTable* sym_tab,
                                        size_t depth) const {
  return INDENT(depth) + "BREAK";
}

std::string via::ir::TrBranch::to_string(const SymbolTable* sym_tab,
                                         size_t depth) const {
  return INDENT(depth) + std::format("BRANCH #{}", target->id);
}

std::string via::ir::TrCondBranch::to_string(const SymbolTable* sym_tab,
                                             size_t depth) const {
...
std::string via::ir::Parameter::to_string(const SymbolTable* sym_tab,
                                          size_t depth) const {
  return INDENT(depth) +
         std::format("{}: {}", SYMBOL(symbol), type.to_string());
}

std::string via::ir::ExprConstant::to_string(const SymbolTable* sym_tab,
                                             size_t depth) const {
  return INDENT(depth) + value.to_string();
}

std::string via::ir::ExprSymbol::to_string(const SymbolTable* sym_tab,
                                           size_t depth) const {
  return INDENT(depth) + std::string(SYMBOL(symbol));
}

std::string via::ir::ExprAccess::to_string(const SymbolTable* sym_tab,
                                           size_t depth) const {
  return INDENT(depth) +
...
std::string via::ir::ExprModuleAccess::to_string(const SymbolTable* sym_tab,
                                                 size_t depth) const {
  return INDENT(depth) +
         std::format("MODULE({})::{}", SYMBOL(mod_id), SYMBOL(key_id));
}

std::string via::ir::ExprUnary::to_string(const SymbolTable* sym_tab,
                                          size_t depth) const {
  return INDENT(depth) + std::format("({} {})", via::to_string(op),
                                     TOSTRING(expr, 0, EXPR_ERROR));
}

std::string via::ir::ExprBinary::to_string(const SymbolTable* sym_tab,
                                           size_t depth) const {
...
std::string via::ir::ExprCall::to_string(const SymbolTable* sym_tab,
                                         size_t depth) const {
...
std::string via::ir::ExprSubscript::to_string(const SymbolTable* sym_tab,
                                              size_t depth) const {
  return INDENT(depth) + std::format("{}[{}]", TOSTRING(expr, 0, EXPR_ERROR),
                                     TOSTRING(idx, 0, EXPR_ERROR));
}

std::string via::ir::ExprCast::to_string(const SymbolTable* sym_tab,
                                         size_t depth) const {
  return INDENT(depth) + std::format("{} AS {}", TOSTRING(expr, 0, EXPR_ERROR),
                                     cast.to_string());
}

std::string via::ir::ExprTernary::to_string(const SymbolTable* sym_tab,
                                            size_t depth) const {
...
std::string via::ir::ExprArray::to_string(const SymbolTable* sym_tab,
                                          size_t depth) const {
...
std::string via::ir::ExprTuple::to_string(const SymbolTable* sym_tab,
                                          size_t depth) const {
  return INDENT(depth) + "<tuple>";
}

std::string via::ir::ExprLambda::to_string(const SymbolTable* sym_tab,
                                           size_t depth) const {
