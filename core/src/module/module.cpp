...
std::expected<via::Module*, std::string> via::Module::load_source_file(
  ...
  Diagnostics diags(path.string(), name, module->m_source);
...
std::optional<const via::Binding*> via::Module::lookup(via::SymbolId symbol) {
  if (auto it = m_defs.find(symbol); it != m_defs.end()) return it->second;
  return std::nullopt;
}
...
std::expected<via::Module*, std::string> via::Module::import(
  ...
  if (!module.has_value()) {
    return std::unexpected(
        std::format("Module '{}' not found", to_string(path)));
  }
