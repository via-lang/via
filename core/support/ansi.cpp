...
std::string via::ansi::format(std::string string, Foreground fg, Background bg,
                              Style style) {
  ...
  if (style != Style::NONE) codes += std::to_string(static_cast<int>(style));
  ...
  if (!codes.empty()) return "\033[" + codes + "m" + string + "\033[0m";
  return string;
}
