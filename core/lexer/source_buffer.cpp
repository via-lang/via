...
via::SourceLoc via::SourceBuffer::location(const char* begin,
                                           const char* end) const {
  size_t lbegin = begin - this->begin();
  return {lbegin, lbegin + (end - begin)};
}

via::SourceLoc via::SourceBuffer::location(const Token& token) const {
  return location(token.lexeme, token.lexeme + token.size);
}
