...
use viac_ast::place::Place;
use viac_ast::ty::Ty;
use viac_ast::value::Value;
...
#[test]
fn value_reference() {
    assert_matches!(parse_expr("&a"), Ok(Expr::Value(Value::Reference(r))) => {
        assert!(!r.strong);
        assert!(!r.mutable);
        assert_matches!(*r.expr.node, Expr::Place(Place::Symbol(_)));
    });
    ...
    assert_matches!(parse_expr("&mut a"), Ok(Expr::Value(Value::Reference(r))) => {
        assert!(!r.strong);
        assert!(r.mutable);
        assert_matches!(*r.expr.node, Expr::Place(Place::Symbol(_)));
    });
